...
class AccountBankStatementImport(models.TransientModel):
    ...
    qif_date_format = fields.Selection([('month_first', "mm/dd/yy"), ('day_first', "dd/mm/yy"), ('year_first', "yyyy-mm-dd")], string='Dates format', required=True, store=False,
    ...
    def _onchange_data_file(self):
        ...
        self.show_qif_date_format = any(self._check_cbi(base64.b64decode(content)) for content in file_contents)
    ...
    def _check_cbi(self, data_file):
        return data_file.strip().startswith(b'RH')

    def _parse_file(self, data_file):
        if not self._check_cbi(data_file):
        ...
        flow = Flow()
        data_file = data_file.decode('utf8').split("\n")
        flow.readfile(data_file, firstrecordidentifier=62)
        vals_bank_statement = {}
        ...
        for disposal in flow.disposals:
            ...
            for record in mergeColumnRecord(disposal.records, record_index='record_group'):
                ...
                if record['credito_debito'] == 'D':
                    amount = -amount
                total_amount += amount
                if str(62) in record['tipo_record']:
                    ...
                    vals_line['ref'] = record['causale'] + record.get('descrizione', '')
                    #vals_line['unique_import_id'] = unique_val
                    vals_line['payment_ref'] = record['causale'] + record.get('descrizione', '')
                    index += 1
                    total += amount
