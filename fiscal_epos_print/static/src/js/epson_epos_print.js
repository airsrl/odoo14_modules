            ...
            this.fiscalPrinter.onreceive = function(res, tag_list_names, add_info) {
                ...
                if (add_info.fiscalReceiptNumber && add_info.fiscalReceiptAmount && add_info.fiscalReceiptDate && add_info.zRepNumber) {
                    ...
                    if (!order.fiscal_receipt_number) {
                        ...
                        order.fiscal_receipt_amount = round_pr(parseFloat(add_info.fiscalReceiptAmount.replace(',', '.')), sender.env.pos.currency.rounding).toFixed(2);
        ...
        */
        printRecItem: function(args) {
            var tag = '<printRecItem'
                ...
                + ' unitPrice="' + (round_pr(parseFloat(args.unitPrice), this.sender.env.pos.currency.rounding).toFixed(2) || '') + '"'
        ...
        */
        printRecItemAdjustment: function(args) {
            var tag = '<printRecItemAdjustment'
                ...
                + ' description="' + this.encodeXml(args.description || '' ) + '"'
                + ' amount="' + (round_pr(parseFloat(args.amount), this.sender.env.pos.currency.rounding).toFixed(2) || '') + '"'
                ...
                if (l.price >= 0) {
                    if(l.quantity>=0) {
                        xml += self.printRecItem({
                            ...
                            quantity: l.quantity,
                            unitPrice: round_pr(l.full_price, self.sender.env.pos.currency.rounding)?.toFixed(2),
                        ...
                        if (l.discount) {
                            xml += self.printRecItemAdjustment({
                                ...
                                amount: parseFloat(round_pr((l.quantity * l.full_price) - l.price_display, self.sender.env.pos.currency.rounding))?.toFixed(2),
                            });
                        }
                    }
                    else
                    {
                        xml += self.printRecRefund({
                            ...
                            unitPrice: round_pr(parseFloat(l.price), self.sender.env.pos.currency.rounding)?.toFixed(2),
                            department: l.tax_department.code
                        });

                        /* TODO This line of code is added by us, check if it's right
                            According to "ePOS Fiscal Print Solution Development Guide Rev T.pdf"
...
//                        }
                    ...
                    xml += self.printRecItemAdjustment({
                        ...
                        description: l.product_name,
                        department: l.tax_department.code,
                        amount: round_pr(-parseFloat(l.price), self.sender.env.pos.currency.rounding)?.toFixed(2),
                    });
                }
            });
...
//                xml += '<directIO command="1135" data="01' + receipt.lottery_code.padEnd(16, ' ') + '0000" />';
                xml += '<printRecLotteryID operator="1" code="' + receipt.lottery_code + '" />';
            }
            ...
            _.each(receipt.paymentlines, function(l) {
                ...
                if (!has_refund){
                    ...
                    payment: Math.abs(round_pr(parseFloat(l.amount), self.sender.env.pos.currency.rounding)),
                    paymentType: l.type,
