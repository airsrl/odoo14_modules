...
class AccountMove(models.Model):
    ...
    @api.onchange("fiscal_document_type_id")
    def _onchange_fiscal_document_type_id(self):
        if self.fiscal_document_type_id:
            move_type = self.move_type
            if self.fiscal_document_type_id.out_invoice and self.move_type in ['out_invoice', 'out_refund']:
                move_type = "out_invoice"
            elif self.fiscal_document_type_id.in_invoice and self.move_type in ['in_invoice', 'in_refund']:
                move_type = "in_invoice"
            elif self.fiscal_document_type_id.out_refund and self.move_type in ['out_invoice', 'out_refund']:
                move_type = "out_refund"
            elif self.fiscal_document_type_id.in_refund and self.move_type in ['in_invoice', 'in_refund']:
                move_type = "in_refund"

            self.move_type = move_type

    def write(self, vals):
        if 'fiscal_document_type_id' in vals and vals['fiscal_document_type_id'] != self.fiscal_document_type_id.id:
            move_type = self.move_type
            fiscal_document_type_id = self.env['fiscal.document.type'].browse(vals['fiscal_document_type_id'])
            if fiscal_document_type_id.out_invoice and self.move_type in ['out_invoice', 'out_refund']:
                move_type = "out_invoice"
            elif fiscal_document_type_id.in_invoice and self.move_type in ['in_invoice', 'in_refund']:
                move_type = "in_invoice"
            elif fiscal_document_type_id.out_refund and self.move_type in ['out_invoice', 'out_refund']:
                move_type = "out_refund"
            elif fiscal_document_type_id.in_refund and self.move_type in ['in_invoice', 'in_refund']:
                move_type = "in_refund"
            vals.update({'move_type': move_type})
