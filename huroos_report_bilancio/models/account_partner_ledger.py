...
class AccountReport(models.AbstractModel):
    ...
    filter_account_type = [{'id': 'receivable', 'name': _('Receivable'), 'selected': False},
                           {'id': 'payable', 'name': _('Payable'), 'selected': True}]
    filter_hide_zero_balance = False
    ...
    def _do_query_group_by_account(self, options, line_id):
        ...
        if options.get("hide_zero_balance"):
            query += '''HAVING SUM(ROUND(\"account_move_line\".''' + balance_field + ''' * currency_table.rate, currency_table.precision)) != 0'''
