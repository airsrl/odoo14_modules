...
# È inutile ora importare EFatturaOut dato che è un TransientModel, quindi nell'env
...
class WizardExportFatturapa(models.TransientModel):
    ...
    def exportInvoiceXML(self, partner, invoice_ids, attach=False, context=None):
        EFatturaOut = self.env['efattura.out']
        ...
        fatturapa = EFatturaOut.create({
            'wizard': self.id,
            'partner_id': partner.id,
            'invoices': [(6, 0, invoice_ids.ids)],
